...
_RAPHAEL_TRANSLATOR = None


def _translate_for_raphael(text: str, provider=None) -> str:
    """Translate Spanish HUD text into Japanese using the local Ollama bridge.

    Chat may run on NVIDIA/Online, but Raphael voice translation must never
    make a second network request. A tiny local qwen3.5:4b call keeps the
    HUD Spanish while Raphael receives Japanese.
    """
    global _RAPHAEL_TRANSLATOR
    ...
    if spoken in fixed:
        return fixed[spoken]
    if _RAPHAEL_TRANSLATOR is None:
    ...
    translated = _RAPHAEL_TRANSLATOR.send_fast_message(
